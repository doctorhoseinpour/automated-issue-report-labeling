...
In this section, we present the results of our experiments and answer
the four research questions posed in \Cref{sec:01_intro}, one per
subsection.
...
Question consistently has the weakest performance among the three labels at every $k$ in both settings. PS per-class F1 at best $k$ is 0.630 for bug, 0.614 for feature, and 0.541 for question. The corresponding PA values are 0.632, 0.629, and 0.550 (\Cref{fig:vtag-kcurve} right panel). Additionally, questions are more frequently misclassified as bugs: 32\% of question issues are predicted as bugs in both settings, while 18\% (PS) and 17\% (PA) are predicted as features. Since \votag predicts the label with the most accumulated similarity weight, this misclassification asymmetry indicates that question issues are more similar to bugs than to features in the embedding space.

%% I intentionally used this tcolorbox to make it different from our other submission.
\begin{myboxi}
\textbf{Answer to RQ1:} Retrieval alone establishes a non-parametric baseline with a peak macro $F_1$ of 0.604 and bounds the useful number of in-context examples at $k\approx15$. 
\end{myboxi}
...
Every \ragtag\ configuration with $k{\ge}1$ outperforms the zero-shot baseline. At each model's peak, the gap over zero-shot averages +0.076 macro $F_1$ and ranges from +0.055 (Qwen-7B) to +0.087 (Qwen-14B). This shows that LLMs benefit substantially from the most siClassification performancemilar labeled examples for IRC.

Every \ragtag\ configuration outperforms \votag's best result (0.604 at PA $k{=}16$), with the smallest margin from Qwen-3B zero-shot (+0.009 F1) and the largest from Qwen-32B PS at $k{=}12$ (+0.163 F1). These results show that adding the LLM reasoning improves IRC on top of pure retrieval.
...
\begin{myboxi}
\textbf{Answer to RQ2:} Every Retrieval-augmented few-shot prompting (\ragtag) configuration with $k{\ge}1$ consistently outperforms the zero-shot and \votag baselines. The best $k$ grows with model size but plateaus within the $k\approx15$ bound established by \votag\. \ragtag\ is weakest on the question class and frequently misclassifies questions as bugs, motivating a more balanced retrieval method. 
\end{myboxi}
...
The performance gain comes from an increase on the question and bug classes with minimal impact on feature. \Cref{tab:bragtag-results} reports per-class $F_1$ at each method's best $k$. Question $F_1$ improves by 0.030--0.068 across all four model sizes. Bug $F_1$ stays within 0.018 of \ragtag, with a slight loss for Qwen-3B and slight gains for the larger models. Therefore, removing bug neighbors in some cases did not decrease overall bug performance. Feature $F_1$ is essentially unchanged. The question-to-bug misclassification rate drops: at each model's best $k$, \ragtag\ misclassifies 26--36\% of true-question issues as bugs while \bragtag\ reduces this to 19--27\%. Averaged across $k\in[1,15]$, the rate drops from 31--40\% (\ragtag) to 24--36\% (\bragtag). Question-to-feature misclassification is unaffected. This confirms that \bragtag's improvement comes from the targeted reduction of question-to-bug misclassification.
...
\begin{myboxi}
\textbf{Answer to RQ3:} The balanced retrieval method (\bragtag), outperforms \ragtag\ at every $k{\ge}6$. The gain comes from a targeted reduction of question-to-bug misclassifications, raising question $F_1$ by 0.030--0.068 without sacrificing bug or feature performance.
\end{myboxi}
...
We fine-tune all models in both PA and PS settings. \Cref{fig:finetune-comparison}(a) compares macro $F_1$ for fine-tuning in both settings against \ragtag's PS at each model's best $k$. Fine-tuning PA outperforms fine-tuning PS at every model size, by +0.033 (Qwen-3B), +0.084 (Qwen-7B), +0.082 (Qwen-14B), and +0.031 (Qwen-32B) macro $F_1$. This trend matches prior findings on LLM fine-tuning for IRC~\cite{heo2025study}.

\ragtag PS outperforms Fine-Tune PS at every model size, by +0.021 (Qwen-3B) to +0.040 (Qwen-7B) macro $F_1$. This shows that using only 300 labeled issues per project, retrieval-augmented few-shot is more effective than LoRA fine-tuning for IRC.
...
\begin{figure}[t]
    ...
    \caption{Per-(project, model) macro $F_1$ difference between \bragtag\ (PS) and fine-tune (PA), across the 11 projects and four Qwen sizes. Each \bragtag\ row uses that model's best $k$.}
    \label{fig:per-project-diff}
\end{figure}

\bragtag's balanced retrieval narrows this gap: the aggregate (\bragtag\ $-$ fine-tune) macro $F_1$ difference is $-0.010$. However, fine-tuning does not consistently lead across projects. \Cref{fig:per-project-diff} reports the per-(project, model) macro $F_1$ difference between \bragtag\ and fine-tune. \bragtag\ outperforms fine-tune in 16 of the 44 (project, model) cells, winning all four model sizes on \texttt{opencv} and three of four on \texttt{bitcoin}. Similar to \ragtag, \bragtag\ performs better relative to fine-tune at the smallest and largest models: \bragtag\ is statistically tied with fine-tune at Qwen-3B and Qwen-32B while fine-tune leads on the mid-sized ones, with per-model (\bragtag\ $-$ fine-tune) deltas of $+0.006$ [$-0.012, +0.024$] (Qwen-3B), $-0.024$ [$-0.039, -0.009$] (Qwen-7B), $-0.029$ [$-0.044, -0.013$] (Qwen-14B), and $+0.010$ [$-0.005, +0.024$] (Qwen-32B). 
