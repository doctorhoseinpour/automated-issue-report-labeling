...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/approach_diagram.pdf}
    \caption{Overview of the four IRC approaches evaluated in this paper. All approaches share the same embedding and retrieval pipeline (\Cref{sec:retrieval}); they differ in how the retrieved neighbors and the query issue are converted into a label.}
    \label{fig:approach-overview}
\end{figure}

\Cref{fig:approach-overview} provides an overview of the four approaches evaluated in this paper: \ragtag, \bragtag, \votag, and the fine-tuning baseline.
...
\subsection{Balanced \ragtag (\bragtag)} \bragtag\ is a \ragtag\ variant designed to reduce the frequent question$\rightarrow$bug misclassifications, without sacrificing performance on the other two labels (\Cref{sec:ragtag}). We hypothesize that bug-labeled neighbors retrieved for true-question queries reinforce the question$\rightarrow$bug misclassification. Therefore, before prompt construction, \bragtag\ inspects the top-K neighbors and drops the bug-labeled ones when their count is within a small margin $m$ of the question-labeled count ($N_\text{bug} - N_\text{question} \le m$). The full misclassification analysis and the empirical justification for \bragtag\ and margin selection are presented in \Cref{sec:bragtag,sec:ragtag}.
