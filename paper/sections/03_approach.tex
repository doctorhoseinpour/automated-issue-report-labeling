...
\paragraph{Balanced \ragtag.} Balanced \ragtag\ (\bragtag) is a \ragtag\ variant that adds a margin-based filter on the retrieved neighbors before prompt construction. After retrieval, \bragtag\ counts the labels among the top-K neighbors: if at least one bug-labeled neighbor is present and the bug count exceeds the question count by no more than a margin $m$, all bug-labeled neighbors are dropped from the prompt. The remaining neighbors are formatted exactly as in \ragtag. We set $m=3$ throughout; the motivation for this rule and the empirical justification of the margin are derived from the bias-diagnosis analysis in \Cref{sec:bragtag}.
