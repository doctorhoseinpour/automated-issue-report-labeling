...
## S5: tone, text-level accuracy, Threats, exact page fill (2026-09-25)
- Changed:
  - `03_approach.tex` → problem formulation, retrieval, kNN footnote/ties, 8,192 footnote, truncation, output parsing rewritten plainly; **temperature**: "set at 0.1 for deterministic classifications" → "a low temperature (0.1), so that outputs vary little between runs"; **A6.2**: "We adopt the prompt template of these studies verbatim, so that our baseline reproduces their method" → "Following these studies, each training example is an instruction prompt …" (the two papers' templates differ; no claim of verbatim reuse).
  - `04_setup.tex` → **"Heo et al." ×3 → "Heo and Lee"**; **A6.3** provenance ("extends the five-project NLBSE'24 dataset~\cite{kallis2024nlbse} with six projects"); "configurations"/"data scope" → "settings"; "retrieval index holds exactly the issues that fine-tuning trains on"; models and metrics sentences de-inflated.
  - `05_evaluations.tex` → RQ1 PS-vs-PA sentence (the old "as the neighbor overlap leads us to expect" was ambiguous); RQ2 "The margin explains the small values of $k$" → "…the losses at small $k$"; "The gain is a targeted reduction…" → "The gain comes from fewer question-to-bug errors"; precision-loss cause joined with "because"; time sentence reordered.
  - `06_discussion.tex` → two colon patterns removed; "remains underexplored" removed; new *Choosing $k$* implication; per-project clarifier for 11× (guard 4); per-size magnitudes 2.4/2.9 in *Where fine-tuning keeps an edge*; one concrete per-class pair at Qwen-32B in *Error profiles*; zero-shot bias clause in *Question-to-bug confusion*; kNN voting labels fewer questions as bugs than zero-shot (in words).
  - `02_related.tex` → **user instruction:** "our benchmark is balanced …, so imbalance alone does not explain it" removed; now the imbalance cause is followed by our zero-shot observation and a pretraining hypothesis ("Our models favor the bug label even before they see any labeled issue, labeling many questions as bugs at zero-shot (§IV-A); we hypothesize that they learned in pretraining that bug reports outnumber questions."), neither explanation ruled out. Added: Heo and Lee's per-project averaging vs our all-issues scoring (dossier A6.4); pooling helps fine-tuning, as Heo and Lee found, but not RAG; "This study takes both steps" (Dinç and Tüzün's future work: k sweep and neighbor filtering); SetFit-encoder contrast; Logan et al. moved next to the other NLP work.
  - `07_threats.tex` → rewritten, four run-in paragraphs (Internal, Construct, Conclusion, External), commented lines deleted. Includes guard 3 (test-set k; fixed k = 12 → 1.2 points), validation-split margin, prompts/hyperparameters, length limits (94.9% untruncated), invalid outputs + fallback, possible pretraining exposure of public issues, label noise (9%), three runs averaged (kept as the paper stated it), failure-analysis sample scope, mixed GPUs for times at Qwen-14B, bootstrap resamples issues not projects (~10 points per project), no multiple-comparison correction, one model family / embedding / balanced benchmark, the 11× ratio depends on 11 × 300, writing-style variation (cited).
  - `08_conclusion.tex` → "These results build on two steps" → "Two earlier results lead to this one"; future work split into its own paragraph with a reason for each direction.
  - `01_intro.tex` → "so we give the neighbors to the LLM" → "so the neighbors carry label information that the LLM can use".
  - `scripts/paper/tab_results_master.py` + `tables/results_master.tex` → caption "Pooled test-set results…" → "Results (%) … on all 3,300 test issues" ("pooled" now means pooled fine-tuning only).
  - `tables/method_cost.tex` → header "RAM (GB)" → "Mem. (GB)"; caption "RAM is the observed peak GPU memory…" → "Memory is the peak GPU memory…". `numbers_sheet.py` still parses it (rerun gave an unchanged NUMBERS.md apart from the timestamp; reverted).
- Most important rewrites (old → new):
  1. "the temperature is set at 0.1 for deterministic classifications" → "set a low temperature (0.1), so that outputs vary little between runs"
  2. "We adopt the prompt template of these studies verbatim, so that our baseline reproduces their method" → "Following these studies, each training example is an instruction prompt …"
  3. "We use the dataset introduced in Heo et al." → "We use the dataset of Heo and Lee, which extends the five-project NLBSE'24 dataset with six projects"
  4. "…so imbalance alone does not explain it" → imbalance cause + our zero-shot observation + pretraining hypothesis (user instruction)
  5. "The gain is a targeted reduction of question-to-bug errors." → "The gain comes from fewer question-to-bug errors."
  6. "…as the neighbor overlap leads us to expect, so we use PS" → "With largely the same neighbors, the two settings score nearly the same, and retrieving from the target project scores slightly higher …"
  7. "The comparison is starker for a project …: fine-tuned on the same issues, the model scores …" → "For a project that cannot draw on other projects' labeled issues, fine-tuning on its own issues scores 3.8–6.0 points below filtered RAG."
  8. "…but combining classification and validation in one workflow remains underexplored" → "…and a triage workflow could combine such validation with classification."
  9. "We selected this model due to its lightweight architecture, fast performance, and effectiveness …; … which provides efficient high-dimensional similarity retrieval" → "…which is small and fast and works well on issue report text"; FAISS clause dropped.
  10. "Assessing how far our findings generalize … warrants its own dedicated studies" → concrete External-validity items.
- Page state (`pagecheck.py --final`): main text ends page 10, right column, y=696 of 719; **2 free lines**; references start page 11 (top), end page 12 with about 86 body-line equivalents of room. **FINAL TARGET MET** (exit 0). Pages 9–10 checked visually: no stranded heading, right column of page 10 reaches the bottom margin. Build: only the known `\balance` overfull vbox; no undefined references. After the tone pass alone, 45 lines were free; filled in the brief's order (Related Work contrasts, Threats items, one per-class result, Discussion clarifiers).
- Numbers: all new prose numbers are in NUMBERS.md or already in the paper: 2.4/2.9 (Table II), 79.1/77.5/72.9/66.9 (NUMBERS §1, Qwen-32B rows), 1.2, 94.9%, 9%, 150, about 10 points. No new computed differences.
- Open questions for the user:
  - The construct-validity sentence "we ran the experiments three times and report the average" is kept as the paper stated it (instruction), but it sits beside bootstrap CIs on one prediction set per method. Please confirm that it is accurate; otherwise delete it (it frees about 1 line).
  - "Mem. (GB)" in Table III: OK, or do you prefer "GPU (GB)"?
  - A reviewer may note that fine-tuning's best score over all sizes (78.5% at Qwen-14B, 16.7 GB) is reached with less memory than filtered RAG's best (78.1% at Qwen-32B, 22.3 GB). The paper compares memory per size (guard 7) and does not raise this. No change made.
  - `sclar2023quantifying` (prompt-format sensitivity) is in refs.bib but still uncited; I did not add it (no new papers).
- Notes for S6:
