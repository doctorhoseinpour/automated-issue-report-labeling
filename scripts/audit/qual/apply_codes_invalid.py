...
Codes (refined after reading all 60):
  CONTINUES-AS-BODY  — model continues generating issue-body content (code,
                       logs, stack traces, expected/actual fields) after the
                       <label> prefill, never reaching a label tag.
  OFF-TOPIC-LOOP     — repetitive numeric/character loops (e.g. "5555...",
                       repeated dots), pure degenerate output.
  PARSER-MISS        — output contains a valid label that the regex/XML
                       parser missed because of extra content before it
                       (e.g., "Answer: <label>X</label>", "X\n<label>X</label>").
  EMPTY-LABEL        — output is essentially empty (`<label>`, `.`, `;`).
  INVALID-LABEL-VALUE— well-formed `<label>X</label>` but X is not in the
                       schema (e.g. "docs", "documentation").
...
INV_CODES: dict[tuple, tuple[str, str]] = {
    # --- BRAGTAG ---
    ("BRAGTAG", "ansible_ansible", 232): ("INVALID-LABEL-VALUE", "Output 'docs'; valid format, label not in schema."),
    ...
    ("BRAGTAG", "microsoft_TypeScript", 145): ("EMPTY-LABEL", "Just '<label>' returned, then nothing."),
    ...
    ("BRAGTAG", "microsoft_vscode", 155): ("PARSER-MISS", "'Answer: <label>feature</label>' — correct answer; parser failed."),
    ("BRAGTAG", "microsoft_vscode", 205): ("PARSER-MISS", "'Answer: <label>question</label>' — correct answer; parser failed."),
    ...
    ("BRAGTAG", "tensorflow_tensorflow", 39): ("PARSER-MISS", "'6\\n<label>bug</label>' — bug correct; format malformed."),
    ...
    ("BRAGTAG", "tensorflow_tensorflow", 146): ("INVALID-LABEL-VALUE", "Output 'documentation' (picked from 'Documentation Bug')."),
    ...
    ("RAGTAG", "bitcoin_bitcoin", 274): ("PARSER-MISS", "'Answer: <label>bug</label>' — model emitted a label; parser failed."),
    ("RAGTAG", "dart-lang_sdk", 250): ("CONTINUES-AS-BODY", "Continues with build output and dart paths."),
    ("RAGTAG", "facebook_react", 178): ("PARSER-MISS", "'Answer: <label>feature</label>' — correct answer; parser failed."),
    ("RAGTAG", "facebook_react", 224): ("EMPTY-LABEL", "Just '<label>' returned."),
    ("RAGTAG", "flutter_flutter", 4): ("EMPTY-LABEL", "Just '.' returned."),
    ...
    ("RAGTAG", "kubernetes_kubernetes", 211): ("EMPTY-LABEL", "Just ')\\r\\n```' — no label."),
    ("RAGTAG", "kubernetes_kubernetes", 215): ("PARSER-MISS", "'<label>...assistant\\n<label>bug</label>' — second label has correct answer."),
    ...
    ("RAGTAG", "microsoft_TypeScript", 9): ("EMPTY-LABEL", "';\\r\\n```' — minimal output."),
    ...
    ("RAGTAG", "tensorflow_tensorflow", 39): ("PARSER-MISS", "'6\\n<label>bug</label>' — bug correct; format malformed."),
    ...
    ("RAGTAG", "tensorflow_tensorflow", 172): ("EMPTY-LABEL", "'el\\n```' — minimal output."),
}
