...
% Coauthor evaluation revision (2026-09-21); preserve the author's blue edits.
\definecolor{coauthorcolor}{HTML}{A02060}
\newcommand{\coauthor}[1]{{\color{coauthorcolor}#1}}
\newcommand{\coauthornote}[1]{\coauthor{\textit{[Draft check: #1]}}}
% Set the box text color explicitly so it survives tcolorbox column/page breaks.
\newenvironment{coauthoranswer}{\tcbset{coltext=coauthorcolor}\begin{myboxi}}{\end{myboxi}}
\pdfstringdefDisableCommands{\def\coauthor#1{#1}}
...
%% Submission build: all draft colors off. Set \draftcolorstrue to see the edits again.
\newif\ifdraftcolors
\draftcolorsfalse
\ifdraftcolors\else
  \renewcommand{\fixed}[1]{#1}
  \renewcommand{\coauthor}[1]{#1}
  \colorlet{coauthorcolor}{black}
\fi
...
% After-skip is zero: IEEEtran already appends ":\ \," to run-in headings
% (\@IEEEsectpunct), so any extra skip here widens the gap after the colon.
\makeatletter
...
\title{Retrieval-Augmented Few-Shot Prompting for Issue Report Classification: An Empirical Study}
...
\input{sections/09_data_availability}   % mandatory, directly after Conclusion

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%          References
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Main text fills pages 1-10; references start on page 11.
\clearpage
\balance
