...
In this section, we describe the implementation details of our three proposed retrieval-based IRC
...
\fixed{\ragtag\ applies retrieval-augmented few-shot prompting to \irc. It shows the query's top-$k$ neighbors (\cref{sec:retrieval}) to the LLM as labeled examples. We use the neighbors, rather than randomly chosen labeled issues, because similar examples can outperform random ones~\cite{liu2022makes, yu2023retrieval} and because a vote over the neighbors' labels already reaches 0.595 macro $F_1$ (\votag, \cref{sec:rq1}).}
...
The LLM is instructed to generate the predicted label wrapped in XML tags (e.g. \texttt{<label>bug</label>}). The labels of the presented examples also follow the same format. We use a regular expression to extract the predicted label. \fixed{Outputs that contain none of the three labels are marked as \textit{invalid}.} Finally, models are loaded with Unsloth\footnote{\url{https://unsloth.ai}} for memory efficiency, and the \fixed{temperature} is set at 0.1 for deterministic classifications.
%%%%%%%%%%%%%%%%%%%%%
...
from the output, and \fixed{outputs that contain none of the three labels} are marked as
...
Similar to \ragtag and \bragtag, the models are loaded with Unsloth and the \fixed{temperature} is set at 0.1.
