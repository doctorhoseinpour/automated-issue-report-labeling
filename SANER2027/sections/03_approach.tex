...
In this section, we describe the implementation details of the \knn baseline and our two retrieval-augmented
methods (\rag and \frag) as well as steps for faithfully reproducing the established LoRA fine-tuning baseline~\cite{heo2025study,aracena2025applying}.
...
pipeline shared by \knn, \rag, and \frag.
\Cref{sec:approach-votag} presents \knn, a non-parametric baseline that classifies the query directly from its nearest neighbors. \Cref{sec:approach-ragtag} describes \rag, our few-shot prompting technique that presents the nearest neighbors as
classified examples in the prompt. \Cref{sec:approach-bragtag} elaborates on \frag's balanced retrieval intervention applied to \rag. Finally, \Cref{sec:finetune} details the faithful reproduction of the fine-tuning baseline.
...
The \knn baseline labels the query by a similarity-weighted vote over the labels of its top-$k$ neighbors, following the distance-weighted $k$-nearest-neighbor rule~\cite{cover1967nearest, dudani1976distance}.\footnote{Squared-similarity weighting and uniform majority were also evaluated, but were outperformed by similarity-weighted voting.} Because it uses no LLM, \knn\ measures how much label information the neighbors carry, and its best $k$ bounds the number of examples we show the LLM (\cref{sec:rq1}). It also labels the issues whose LLM output is invalid when we evaluate complete pipelines (\cref{sec:method-comparison}). Formally, for a
...
\rag shows the query's top-$k$ neighbors (\cref{sec:retrieval}) to the LLM as labeled examples. We use the neighbors, rather than randomly chosen labeled issues, because similar examples can outperform random ones~\cite{liu2022makes, yu2023retrieval} and because a vote over the neighbors' labels already reaches 0.595 macro $F_1$ (\knn, \cref{sec:rq1}).
...
Our empirical evaluations in \Cref{sec:ragtag} reveal that \rag has the lowest class-specific $F_1$ on the question label, and it frequently misclassifies true-question queries as bugs, a known \irc\ weakness~\cite{colavito2024impact, colavito2025benchmarking}. Furthermore, LLM zero-shot results also show that the models are already biased towards predicting the bug label for true-question queries even before the examples are presented. Based on these insights, we hypothesize that presenting bug-labeled examples for true-question queries likely reinforces the question-to-bug misclassification trend. To address this, we propose a more balanced retrieval intervention (\frag) that removes the bug-labeled neighbors whenever the query is a suspected question based on its nearest neighbors' label distribution. \Cref{sec:ragtag} shows that true-question queries retrieve almost balanced bug and question neighbors. Due to this balanced distribution, \frag\ removes the bug-labeled examples from the prompt whenever $N_\text{bug} - N_\text{question} \le m$, where $N_\text{label}$ is the count of neighbors with that label in the top-$k$ and $m$ is a small positive margin. The margin is empirically calibrated on a holdout validation set. The empirical motivations for \frag and its margin calibration are fully detailed in \Cref{sec:ragtag}. Other than the balanced retrieval intervention, \frag inherits \rag's full setup and configurations.
