...
\label{sec:problem}
Given a query issue report, we assign it exactly one of three labels: \emph{bug}, \emph{feature}, or \emph{question}. We use only the issue's title and description, with no other signals from the issue tracker. The label set follows prior IRC studies~\cite{aracena2024applyinglargelanguagemodels,aracena2025applying,izadi2022catiss,kallis2019ticket,antoniol2008bug}. 

\subsection{Indexing and Retrieval}
\label{sec:retrieval}
We embed each labeled issue's title and description and keep its label as metadata, outside the embedding. For the embeddings, we use the \texttt{all-MiniLM-L6-v2} model from the Sentence-Transformers library~\cite{reimers2019sentence}, which is small and fast and works well on issue report text~\cite{huang2025back}. We index the embeddings with the FAISS library~\cite{douze2025faiss}. At retrieval, the query issue is embedded with the same model, and FAISS returns the top-$k$ indexed issues by cosine similarity to the query (its nearest neighbors), in descending order of similarity.

%%%%%%%%%%%%%%%%%%%%
\subsection{$k$NN Voting Baseline}
\label{sec:approach-votag}
The \knn baseline labels the query by a similarity-weighted vote over the labels of its top-$k$ neighbors, following the distance-weighted $k$-nearest-neighbor rule~\cite{cover1967nearest, dudani1976distance}.\footnote{Squared-similarity weighting and an unweighted majority vote scored lower.} Because it uses no LLM, \knn\ measures how much label information the neighbors carry. Its peak sets the largest $k$ we give the LLM (\cref{sec:rq1}), and it labels invalid LLM outputs in the fallback (\cref{sec:rq3}). Formally, for a
...
where $\mathcal{L}$ is the label set, and $\mathbf{1}[l_i = c]$ is the indicator function that equals $1$ when neighbor $i$ has label $c$ and $0$ otherwise. For each label, \knn\ sums the cosine similarities of the neighbors that carry that label, and the $\operatorname*{arg\,max}$ operator returns the label with the largest total sum. Ties go to the tied label of the most similar neighbor.
...
\rag shows the query's top-$k$ neighbors (\cref{sec:retrieval}) to the LLM as labeled examples. We use the neighbors, rather than randomly chosen labeled issues, because similar examples can outperform random ones~\cite{liu2022makes} and because the neighbors' labels alone reach 59.5\% macro $F_1$ (\cref{sec:rq1}).
...
We evaluate $k \in \{0, 1, 3, 6, 9, 12, 15\}$, up to \knn's peaks at $k{=}15$--16 (\cref{sec:rq1}). The maximum prompt sequence length is set to 8,192 tokens.\footnote{In preliminary experiments, 8,192 tokens gave the best trade-off between macro $F_1$ and inference speed.} Roughly 30\% of this length is reserved for the query issue and 70\% for the examples. Examples that exceed their share are truncated in proportion to their length, and so is a query that exceeds its share. Truncation removes text from the end of a description and keeps all titles and labels.
The LLM is instructed to wrap its label in XML tags (e.g., \texttt{<label>bug</label>}), the format in which the examples show their labels. A regular expression extracts the label, and outputs that contain none of the three labels are \textit{invalid}. We load the models with Unsloth\footnote{\url{https://unsloth.ai}} to reduce memory use and set a low temperature (0.1), so that outputs vary little between runs.
%%%%%%%%%%%%%%%%%%%%%
...
\label{sec:finetune}
Prior LLM-based \irc\ studies fine-tune open LLMs on labeled issue reports with LoRA~\cite{hu2022lora, heo2025study, aracena2025applying}. Following these studies, each training example is an instruction prompt with the issue's title and description as input and its label as output. At inference, the fine-tuned model receives the query issue in the same format and completes the empty label field. A regular expression extracts the label, and outputs that contain none of the three labels are \textit{invalid}.

Training hyperparameters follow prior work where it reports them: LoRA rank and alpha of 16 and a 2,048-token limit~\cite{aracena2025applying}, and 3 epochs with a per-device batch size of 1~\cite{heo2025study}. We use a learning rate of $2 \times 10^{-4}$, the paged AdamW 8-bit optimizer~\cite{dettmers2023qlora}, and 16 gradient-accumulation steps. The 2,048-token limit leaves 94.9\% of the test issues untruncated, and longer issues lose the end of their description.
As for the RAG variants, we load the models with Unsloth and set the temperature to 0.1.
