...
\begin{abstract}
Classifying issue reports helps with software maintenance and triage. Fine-tuned large language models (LLMs) give the best reported results, but learning from new labels or adopting a newer model requires retraining. Retrieval-augmented few-shot prompting (RAG) needs no training but, to our knowledge, has not been systematically evaluated for this task or compared with Low-Rank Adaptation (LoRA) fine-tuning. To fill this gap, we conduct an empirical study with four Qwen2.5-Instruct models and 6{,}600 issue reports from eleven projects. We first test whether a query issue's nearest neighbors predict its label by taking a similarity-weighted vote over their labels (\knn), which reaches 0.595 macro~$F_1$. \rag therefore gives these neighbors to the LLM as few-shot examples. After observing that \rag\ most often mislabels questions as bugs, we propose \frag, which drops bug-labeled examples for suspected questions. With the same 300 labeled issues per project, \rag\ and \frag\ at their best number of examples outperform LoRA fine-tuning of the same models by 0.021--0.060 macro~$F_1$ at every size. When fine-tuning instead uses all eleven projects' labeled issues, it leads \rag\ by 0.028 on average and \frag\ by 0.010, a difference that is not significant. With \knn replacing invalid LLM outputs, \frag\ matches fine-tuning (+0.001 on average). Needing only the target project's labeled issues and 33\% less peak GPU memory on average, retrieval-augmented few-shot prompting is a competitive, training-free alternative to LoRA fine-tuning.
\end{abstract}
